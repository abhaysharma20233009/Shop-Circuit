...
exports.createProduct = catchAsync(async (req, res, next) => {
  console.log("bodyyyy " + req.body); // Fixed syntax
  console.log(req.user);
  
  const { productName, price, numberOfItems, description, sellerType, category } = req.body;
  const productImage = req.file ? req.file.path : null; // Get image path

  // Save to database...
  res.status(201).json({ message: "Product created successfully!", imageUrl: productImage });
