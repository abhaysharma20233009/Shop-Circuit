...
exports.getAllProducts = catchAsync(async (req, res, next) => {
  ...
  const features = new APIFeatures(
    Product.find({status:'pending'}).populate("sellerId", "shopName shopAddress contactNumber sellerType"),
    req.query
  ).filter().sort().limitFields().paginate();

  const products = await features.query.exec();
  const productsWithSellers = products
    .filter(product => product.sellerId.sellerType === "shopkeeper") // Fix: Access sellerType correctly
    .map(product => ({ ...product.toObject() })); // Simplified map function
