...
exports.markApproveRentRequest = async (req, res) => {
  try {
    ...
    // Delete sell request after approving
    await RequestedRent.findByIdAndDelete(requestId);

    res.status(200).json({ message: 'Rent request marked approved' });
  } catch (error) {
    res.status(500).json({ message: 'Error approving sell request', error });
  }
};
