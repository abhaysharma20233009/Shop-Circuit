...
export default function AllSells() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await fetch("http://localhost:3000/api/v1/products/sell/", {
          credentials: "include",
        });
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {products.slice(0, visibleProducts).map((product) => (
          ...
          >
            ...
            <p className="text-gray-800 font-medium">Seller: {product.username}</p>
            {product.sellerDetails && (
              <div className="mt-2 text-gray-700">
                <p>Hostel: {product.sellerDetails.hostelName}</p>
                <p>Room: {product.sellerDetails.roomNumber}</p>
              </div>
            )}
          </div>
        ))}
